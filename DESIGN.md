...
| [components/my-stats.css](public/css/components/my-stats.css) | „Meine Statistik"-Karte (aggregierte Schreib-Werte über alle eigenen Bücher). `.mystats-hero` / `.mystats-hero-cell` / `.mystats-hero-value` / `.mystats-hero-label` — Akzent-Panel mit zwei grossen serifen Kennzahlen (Zeichen, Normseiten) in `var(--card-accent)` (Hue `--card-accent-mystats`). `.mystats-substats` / `.mystats-substat(-value/-label)` — kompakte Sekundär-Zeile. `.mystats-allbooks-note` (Hinweis „über alle Bücher"). `.mystats-chart-section` / `.mystats-section-label` (Eyebrow) / `.mystats-controls` / `.mystats-chart-wrap` (240px) für den Entwicklungs-Chart (Chart.js lazy; Modus-`.btn-group` Gesamt/Pro Buch + Metrik-Combobox + Zeitraum-`.btn-group`, mirror von `.book-stats-chart-wrap`). Pro-Buch-Modus: eine Linie je Buch aus fester JS-Farbpalette, Legende unten. Eckig, Tabular-Nums. |
