...
- **Home-Klick / View-Reset Pflicht** ([public/js/app-view.js](public/js/app-view.js)): jede neue Karte braucht drei synchron gehaltene Stellen, sonst bricht der Home-Klick (`<h1 @click="resetView()">`):
  1. `_closeOtherMainCards(keep)` — neue Zeile `if (keep !== 'xxx') this.showXxxCard = false;` (Exklusivität bei Seitenklick / anderem Karten-Open).
  2. `resetView()` — explizit `this.showXxxCard = false;` setzen. Ohne Reset bleibt Flag true, `_maybeOpenBookOverview` skipt → Buch-Übersicht öffnet nicht beim Home-Klick.
  3. `_maybeOpenBookOverview()` — neue Karte in `anyOpen`-Disjunktion ergänzen, sonst wird Übersicht trotz offener Karte als Default-Home eingeblendet.
  Drei Stellen sind drift-anfällig (eine vergessen = stille Regression). Test: [tests/unit/card-exclusivity.test.mjs](tests/unit/card-exclusivity.test.mjs) erweitern.
