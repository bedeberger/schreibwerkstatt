...
- [docs/buchchat-tools.md](docs/buchchat-tools.md) — Agentic Buch-Chat: Tool-Inventar (33 Stück inkl. `final_answer`-Endpunkt + `generate_image`), `ctx`-Vertrag, Truncation, Loop-Constraints, neues Tool anlegen.
...
- [docs/share-link.md](docs/share-link.md) — Share-Link (Page/Chapter/Book public via opaken Token): SSR-Reader-View ohne Alpine, Mount **vor** Auth-Guard, In-Memory-Rate-Limit + Honeypot, IP-Hash für GDPR, Owner-Karte listet nur Links (Unread-Tracking via `owner_last_seen_at`), „Kommentare anzeigen" wechselt in die Editor-Ansicht; Kommentar-Leisten (Notebook-Leseansicht + Bucheditor, geteilter Kern `comment-rail-core.js`) zeigen verankerte **und** allgemeine Leser-Kommentare (Margin-Rail, `/share/api/book-comments/:book_id`, Grid-Flag `pageCommentRailOpen`).
