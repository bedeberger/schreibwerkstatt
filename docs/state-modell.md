...
4. **Chat-Modus** (showChatCard) snapshotet `checkDone` in `_checkDoneBeforeChat` und setzt `checkDone=false` ([cards/chat-card.js](../public/js/cards/chat-card.js) `onShow` — nur Seiten-Chat, die geteilte `chat-base.js` fasst `checkDone` nicht an); beim Schliessen Restore ([app-view/cards.js](../public/js/app/app-view/cards.js)). Ohne diesen Snapshot würde der Chat Findings doppelt rendern.
