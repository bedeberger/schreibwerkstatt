...
- **`share_links`** — Token-PK, `kind IN ('page','chapter','book')` (`'book'` ab Migration 213), FKs auf `books`/`pages`/`chapters`/`app_users` (alle `ON DELETE CASCADE`), `intro` (Plaintext, max 2000 Zeichen), `expires_at`/`revoked_at`/`view_count`/`owner_last_seen_at`. CHECK erzwingt je nach `kind` genau das passende Ref-Set (Sentinel-frei): `page` → `page_id` gesetzt, `chapter` → `chapter_id` gesetzt, `book` → **beide** NULL (Buch via `book_id`).
