...
- **Sprung in die Lese-/Edit-Oberfläche (Owner):** Klick auf einen verankerten Kommentar (`gotoComment`) springt scope-abhängig: **Page-Share → Notebook-Editor** auf `link.page_id` + transientes Highlight (`::highlight(share-comment-jump)`, nach 6 s entfernt); **Buch-/Kapitel-Share → Bucheditor** (`book-editor:goto-comment`-Event mit `data-bid`) — dort zeigt die Bucheditor-Kommentar-Leiste alle Buch-Kommentare und öffnet/scrollt zum Thread (kein `/locate` mehr nötig, der ganze Stream ist sichtbar). Die Re-Anchor-Logik (`locateRange`/`resolveCurrentQuote`) liegt als SSoT in [public/js/share-anchor.js](../public/js/share-anchor.js) und wird von Reader, Notebook-Leiste, Bucheditor-Leiste **und** Owner-Karte geteilt.

### Kommentar-Leiste im Bucheditor

Der Bucheditor ([book-editor.md](book-editor.md)) zeigt verankerte Leser-Kommentare des **ganzen Buchs** als rechte Margin-Rail (dritte Layout-Spalte: Outline | Stream | Kommentare). Pendant zur Notebook-Read-Modus-Leiste, aber Scope = ganzer `.book-editor-stream` statt Einzelseite. Eigenständiges Methoden-Modul [public/js/editor/book-editor-comments.js](../public/js/editor/book-editor-comments.js) (in `bookEditorCard` gespreadet), reuse der geteilten Bausteine (`groupThreads`/`locateRange`/`resolveCurrentQuote`/`renderInline`/`loadDiff`) und der `.comment-rail*`-Optik. Threads, Owner-Reply/Resolve/Delete + „Stelle geändert"-Diff identisch zur Notebook-Leiste; Daten aus `/share/api/book-comments/:book_id`. Kein Eingriff in die Notebook-Leiste.
