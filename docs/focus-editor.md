...
- **`enterFocusMode` Pflicht-Sequenz** ([card.js:45-82](../public/js/editor/focus/card.js#L45-L82)): `_flushDraftSaveNow` → `focusActive=true` + body-class → `$nextTick` → `_focusInstall` → `_focusUpdateActive(true)` → `writeFocusSnapshot`. Fehler im Install → `_focusTeardown` + Snapshot-clear + Flag zurück, State zurück auf `idle`.
