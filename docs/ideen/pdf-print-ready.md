...
> **Architektur-Update (überholt Phase-1-Verortung unten):** Die Phase-1-Titelei (ISBN, Subtitle, Jahr, Widmung, Impressum, Copyright, Frontmatter/Motto, Autor-Bio + Cover + Autorfoto) lebt **nicht** mehr profil-gebunden in `pdf_export_profile.config.extras` / `pdf_export_profile.cover_image`/`author_image`, sondern **buch-weit in `book_publication`** (1:1 zu `books`, Migration 166) — Single Source of Truth für PDF **und** EPUB. Gepflegt im **BookSettings → Publikation-Tab**, nicht in der PDF-Export-Karte. Der PDF-Render-Job spiegelt diese Metadaten vor dem Render in `config.extras`. Details: [docs/publikation-export.md](../publikation-export.md). Die alte Migration `N` (author_image-BLOB auf `pdf_export_profile`) wurde durch Migration 166 ersetzt; die profil-gebundenen `cover_image`/`author_image`-Spalten bleiben nur als Migrations-Seed-Quelle bestehen. **Profil-spezifisch bleiben** (zu Recht): Layout/Print/Fonts/TOC/Chapter, `print`-Block (Bleed/Crop/Trim/K-only), `pdfa.standard`, Rückseiten-Bild (Phase-4-Cover-PDF), `barcode`/`imprintPosition`-Render-Toggles.
