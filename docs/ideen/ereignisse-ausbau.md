...
- ~~**Legacy-toter-Code in remap.js:81+84**~~ — erledigt: Dedup-Key auf `(datum_year, datum_month, datum_day, ereignis.toLowerCase())` umgestellt, toter `parseInt(ev.datum)`-Sort entfernt.
...
**Cleanup** ~~(Mini-Aufgabe)~~ — erledigt: toter `parseInt(ev.datum)`-Sort + `ev.datum`-Dedup-Key in remap.js entfernt, Dedup auf `(year, month, day, ereignis.toLowerCase())` umgestellt.
...
**Cleanup ~~(jederzeit, klein)~~ — erledigt:** [routes/jobs/komplett/remap.js:81](../../routes/jobs/komplett/remap.js#L81) — `ev.datum`-Dedup-Key auf `(datum_year, datum_month, datum_day, ereignis.toLowerCase())` umgestellt, toter `parseInt(ev.datum)`-Sort entfernt. `updateFigurenEvents` schreibt `sort_order = j` (Schleifen-Index = KI-Reihenfolge), DB-Lesepfad sortiert per ORDER BY auf strukturierten Feldern.
