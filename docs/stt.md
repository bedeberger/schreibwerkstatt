...
- **Anker ans Ende beim Mic-Klick:** `_sttStart` ruft `_sttAnchorToEnd()` — Caret wird beim Aufnahmestart ans Editorende gesetzt (+ Scroll dorthin), damit Diktat unten anwaechst statt an einer evtl. veralteten Caret-Position mitten im Text. Folgesegmente haengen am vorrueckenden Caret weiter an.
