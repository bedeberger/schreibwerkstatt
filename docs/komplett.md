...
Schema + Regeln liegen im **System-Prompt** (`SYSTEM_KOMPLETT_*_BLOCKS`) → über alle Kapitel-Calls gecacht. Szenen/Assignments **und Songs** (`figuren_namen`) verwenden **Klarnamen** statt fig_id-Referenzen (ID-Remap erst nach P2/P3). **Why Songs auf Namen:** der Song-Extraktions-Pass (Claude B) läuft parallel zu A1 und kennt dessen fig_ids nicht — eine ID-Referenz wäre nur geraten (Beschreibung nennt Figur X, `figuren` verlinkt zufällige ID Y). Nur Orte referenzieren Figuren noch per fig_id (gleiche Schwäche, noch nicht migriert).
...
- **P3-Songs:** KI-Call nur wenn überhaupt Songs extrahiert wurden. Figuren-Verknüpfung über **`figuren_namen`** (Klarnamen), die `runPhase3Songs` nach P2 via `figNameToId`/`figNameToIdLower` (exakt + lowercase-Fallback, Helper `_remapFigNames`) zu kanonischen `fig_id` auflöst — nicht auflösbare Namen werden verworfen. `saveSongsToDb` bekommt fertige `fig_id`s (unverändert). Kein `idRemap`/`validFigIds`-Pfad mehr (das bleibt Orten vorbehalten).
