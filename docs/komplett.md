...
- **Anachronismus-Prüfung (nur bei `book_settings.zeitlinie_real`):** `buildAnachronismusData` (in `job.js`) leitet die Erzählzeit-Spanne aus sicher datierten Figuren-/Zeitstrahl-Ereignissen (`datum_unsicher=0`, `datum_year`) ab und sammelt die im Buch erwähnten Songs (`songs`) und Welt-Fakten (`world_facts`, Kategorien `technik`/`historie`/`ereignis`/`kultur`). Beides geht via optionalem Argument in `buildKontinuitaetSinglePassPrompt`/`buildKontinuitaetCheckPrompt` (`_buildAnachronismusBlock` + `_ANACHRONISMUS_RULE`); das Modell vergleicht das reale Erscheinungs-/Erfindungs-/Ereignisjahr (eigenes Wissen, keine externe Quelle, keine DB-Jahres-Spalte) gegen die Erzählzeit und meldet Treffer als `typ: 'anachronismus'`. Ohne echte Zeitlinie oder ohne datierte Ereignisse liefert `buildAnachronismusData` `null` → der Block entfällt komplett, Verhalten unverändert. Lesepfad-sicher: `figure_events`/`songs`/`world_facts` sind vor P8 bereits persistiert.
