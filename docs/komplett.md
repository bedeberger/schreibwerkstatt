...
- **P3 Orte:** Single-Pass direkt übernommen; Multi-Pass KI-Konsolidierung. Figuren-Verknüpfung über **`figuren_namen`** (Klarnamen), die `runPhase3` nach P2 via `figNameToId`/`figNameToIdLower` (exakt + lowercase-Fallback, Helper `_remapFigNames`) zu kanonischen `fig_id` auflöst → `location_figures`; nicht auflösbare Namen verworfen. Bei Claude-Multi-Pass läuft der Orte-Call **parallel zu P2** mit prelim-figurenKompakt (nur als Namens-Hinweisliste im Prompt), Namens-Auflösung post-hoc.
