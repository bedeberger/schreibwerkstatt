...
- **Per-Job-Modell (`ai.claude.model.komplett`, leer = folgt `ai.claude.model`, nur `ai.provider = claude`):** überschreibt das Claude-Modell für die gesamte Komplettanalyse-Familie (P1–P8 + `runKontinuitaetJob`), z.B. Opus für die gründlichere Extraktion, während global Sonnet (Lektorat) läuft. Durchgereicht via ALS-Context (`setContext({ claudeModel })`) an [lib/ai.js](../lib/ai.js)#`_resolveClaudeModel`; das Modell fliesst in die `cacheVersion` ein (Modellwechsel invalidiert die Caches). `lib/ai.js` lässt `temperature` für Opus 4.7+ automatisch weg (sonst HTTP 400; `_claudeAcceptsTemperature`).
