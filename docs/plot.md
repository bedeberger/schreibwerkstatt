...
- **`plot_beats`** — Karten in einer Zelle (Akt × Strang), geordnet via `sort_order` **pro Zelle** (`act_id` + `thread_id`). `thread_id` (SET NULL) ist die Strang-Zuordnung (`NULL` = „ohne Strang"-Lane; null Stränge → flaches Board). `status ∈ {geplant, entwurf, im_buch, verworfen}` (CHECK-Constraint) hält „geplant → schon geschrieben" nach. `chapter_id` (SET NULL) verknüpft den Beat mit dem Zielkapitel im Manuskript.
...
|---------|----------|---------|---------|--------|
| `plot-brainstorm`  | `POST /jobs/plot-brainstorm`  | `${bookId}\|brainstorm\|${actId}\|${threadId\|'none'}` | `{ book_id, act_id, thread_id? }` | `{ vorschlaege:[{label, begruendung}], actId, threadId }` |
