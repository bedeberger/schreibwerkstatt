...
**Zeilen:** `threadLanes()` = Stränge (Position-Reihenfolge) + die „ohne Strang"-Lane (`id: null`) immer am Ende. Pro Zeile sticky linkes Label (`.plot-swim-lane`, `position: sticky; left: 0`) + je Akt eine Zelle (`.plot-swim-cell`), gefüllt aus `filteredBeatsForCell(actId, threadId)`. Horizontaler Scroll via `min-width: max-content`. CSS: [plot-swimlane.css](../public/css/book/plot-swimlane.css) (ergänzt plot-board.css; Beat-/Add-/Farb-/Brainstorm-/Intensitäts-Styles wiederverwendet).
...
- **Strang löschen ≠ Beats löschen** — `plot_beats.thread_id` ist `SET NULL`; gelöschte Stränge entkoppeln nur, die Beats fallen in die „ohne Strang"-Lane.
