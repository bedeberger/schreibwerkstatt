...
**Welt-Fakten-Block** ([public/js/prompts/chat.js](../public/js/prompts/chat.js)#`buildWeltfaktenBlock`): die etablierten Welt-Fakten des Buchs (`world_facts`) im **stabilen Block 1**, gedeckelt über `weltfaktenBlockChars` ([routes/jobs/chat/shared.js](../routes/jobs/chat/shared.js), 10 % des Input-Budgets, Minimum 2000 Zeichen — kein eigenes App-Setting, gleiche Ableitung wie der Figuren-Deckel), im agentischen Pfad zusätzlich absolut gedeckelt auf `AGENT_WELTFAKTEN_MAX_CHARS` ([routes/jobs/chat/book-chat.js](../routes/jobs/chat/book-chat.js)). **Why:** auf einem 1M-Fenster griffen die 10 % nie, und ein ausanalysiertes Buch trug mehrere hundert Fakten in jede Iteration; den Rest holt `list_world_facts` gezielt. **Why Block 1 und nicht Erst-Kontext:** die Fakten hängen am Buch, nicht an der Frage — im ungecachten Erst-Kontext-Block würde derselbe Inhalt jede Iteration neu bezahlt. Zwei Regeln: was der Deckel geschluckt hat, wird im Block ausgewiesen (die Weltgesetze `regel`/`technik` stehen vorn, damit zuerst das Beiwerk fällt), und ein **nicht erhobener** Index liefert **keinen** Block — ein leerer würde als «diese Welt hat keine Regeln» gelesen (dass der Index fehlt, sagt `list_world_facts`, wenn der Agent fragt).
