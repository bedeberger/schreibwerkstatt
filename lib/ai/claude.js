...
async function _callClaude(messages, systemPrompt, onProgress, maxTokensOverride, signal, cacheLastMessage) {
  const maxAttempts = _retryMaxAttempts();
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    try {
      return await _callClaudeAttempt(messages, systemPrompt, onProgress, maxTokensOverride, signal, cacheLastMessage);
    } catch (e) {
...
async function _callClaudeAttempt(messages, systemPrompt, onProgress, maxTokensOverride, signal, cacheLastMessage) {
