 ...
 *   { available: true, passed, errors, warnings, fatals, items, report }   — Standard-Fall
 *     items: [{ id, severity, message, path, line, column }] — normalisierte Einzelmeldungen
...
async function validateEpub(buffer) {
  ...
  try {
    ...
    for (const m of messages) {
      ...
      // Erste Location reicht für die Diagnose (path:line:col); die übrigen
      // werden über additionalLocations angedeutet.
      const loc = Array.isArray(m?.locations) ? m.locations[0] : null;
      items.push({
        id: m?.ID || null,
        severity: sev || null,
        message: String(m?.message || '').replace(/\s+/g, ' ').trim(),
        path: loc?.path || null,
        line: typeof loc?.line === 'number' && loc.line >= 0 ? loc.line : null,
        column: typeof loc?.column === 'number' && loc.column >= 0 ? loc.column : null,
      });
    }
    const passed = fatals === 0 && errors === 0;
    return { available: true, passed, errors, warnings, fatals, items, report };
  } finally {
