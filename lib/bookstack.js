...
/**
 * DELETE /api/<path>. BookStack antwortet mit 204 No Content; daher keine
 * JSON-Response. Liefert `true` bei Erfolg, wirft bei !ok einen Error mit
 * `status` und `bodyText`.
 */
async function bsDelete(path, token, { timeoutMs = 30000 } = {}) {
  const resp = await fetch(`${BOOKSTACK_URL}/api/${path}`, {
    method: 'DELETE',
    headers: { Authorization: authHeader(token) },
    signal: AbortSignal.timeout(timeoutMs),
  ...
  const err = new Error(`BookStack DELETE /api/${path}: HTTP ${resp.status}`);
...
async function bsGetAll(path, token, opts) {
  ...
  while (true) {
    ...
    const fullUrl = `${path}${sep}count=${COUNT}&offset=${offset}`;
    const data = await bsGet(fullUrl, token, opts);
    console.log('[bsGetAll DBG]', fullUrl, 'total:', data.total, 'data.length:', (data.data||[]).length, 'tokenId:', (token?.id||token?.token_id||'').slice(0,6));
...
module.exports = { bsGet, bsPost, bsPut, bsDelete, bsGetAll, bsBatch, authHeader, BOOKSTACK_URL };
