...
// Chat-Typen ausgeschlossen: ihr Verbrauch wird unten aus chat_messages summiert.
// Ohne Ausschluss zaehlte das Budget-Gate jeden Chat-Dollar doppelt → User
// erreichten ihr Hard-Limit bei der Haelfte des echten Verbrauchs.
const _stmtJobRowsForUser = db.prepare(`
  SELECT provider, model, tokens_in, tokens_out, cache_read_in, cache_creation_in, cache_creation_1h_in
    FROM job_runs
   WHERE user_email = ? AND queued_at >= ? AND ${excludeChatSourcedSql()}
`);
