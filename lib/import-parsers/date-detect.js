...
const PATTERNS = [
  ...
  // — sonst zu viele false positives ("Tag 2 Notiz 5" waere ambig).
  {
    name: 'DD-anywhere',
    test: f => {
      // Doppel-Ext (z.B. "persoenliches_03.odt.docx") aufloesen: solange noch
      // eine Endung uebrig ist und der Body Buchstaben enthaelt, weiter strippen.
      let stripped = _stripExt(f);
      while (/\.[a-zA-Z]+$/.test(stripped)) stripped = _stripExt(stripped);
      // Lookarounds statt \b — Underscore zaehlt als \w, daher kein word-boundary
      // zwischen "_" und "03"; \b matched dort nicht.
      const all = [...stripped.matchAll(/(?<!\d)(\d{1,2})(?!\d)/g)].filter(m => {
        const n = +m[1];
        return n >= 1 && n <= 31;
      });
...
// Erste Heading-Zeile (h1/h2/h3) aus HTML — bevorzugt fuer Titel-Extraktion
// bei Dokumenten ohne Datum im Pfad. Strippt Inline-Tags, gibt Text zurueck.
function firstHeadingFromHtml(html) {
  if (!html) return '';
  const m = /<h[1-3][^>]*>([\s\S]*?)<\/h[1-3]>/i.exec(String(html));
  if (!m) return '';
  return m[1].replace(/<[^>]+>/g, '').replace(/&nbsp;/g, ' ').trim();
}

// Filename → Thema-Token. Endungen strippen (auch doppelt), Trenner zu Spaces,
// Tag-Zahlen am Ende abschneiden ("persoenliches_03" → "persoenliches").
function titleFromFilename(filename) {
  if (!filename) return '';
  let s = String(filename);
  while (/\.[a-zA-Z]+$/.test(s)) s = s.replace(/\.[a-zA-Z]+$/, '');
  // Trenner → Space
  s = s.replace(/[_\-]+/g, ' ').replace(/\s+/g, ' ').trim();
  // Tageszahl am Ende (oder Anfang) abschneiden, wenn sie das einzige Zahlen-
  // Element war — bei Filenames wie "Persoenliches 16" oder "16 Notiz".
  s = s.replace(/\s+\d{1,2}$/, '').replace(/^\d{1,2}\s+/, '').trim();
  // Pure-Zahlen (auch durch Whitespace getrennt — z.B. ISO-Datums-Filenames
  // wie "2024-05-04" werden zu "2024 05 04" gestripped, bleiben aber pure
  // Ziffern) sind kein Titel.
  if (!s || /^[\d\s]+$/.test(s)) return '';
  return s;
}

// Titel fuer month-only-Eintraege. Reihenfolge:
//   1. Erste Heading (h1-h3) aus HTML
//   2. Filename-Body (Trenner aufgeloest, Tag-Nummer abgeschnitten)
//   3. Erste Text-Zeile aus HTML (Fallback fuer Dokumente ohne Heading)
// Gibt '' zurueck, wenn nichts brauchbares gefunden wurde.
function extractTitle(html, filename) {
  const heading = firstHeadingFromHtml(html);
  ...
  const firstLine = firstLineFromHtml(html);
...
module.exports = {
  ...
  scoreSample, parseMonthToken, PATTERNS,
};
