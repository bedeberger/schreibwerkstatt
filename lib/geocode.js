...
// vor. Genutzt von der HTTP-Route (routes/geocode.js, reiner Heuristik-Lookup)
// UND vom KI-Resolve-Job (routes/jobs/geocode.js) als Geocoding-Schritt nach der
// KI-Normalisierung. Beide teilen Provider, Rate-Limit-Kette und Normalisierung.
//
  ...
  return feats
    .map(f => {
      ...
      const displayName = [p.name, p.street, p.city, p.state, p.country]
        ...
        .join(', ');
      // Laendercode (ISO-3166-1-alpha-2, lowercase) fuer den Post-Filter — Photon
      // kennt keinen countrycodes-Anfrageparameter.
      const countrycode = /^[A-Za-z]{2}$/.test(String(p.countrycode || '')) ? String(p.countrycode).toLowerCase() : null;
      return { lat, lng, displayName, countrycode };
    })
    .filter(Boolean);
}

// Bevorzugt Kandidaten im Zielland (cc, ISO-2 lowercase): gibt es Treffer im
// Land, nur diese; sonst die ungefilterte Liste. Ersatz fuer Nominatims
// countrycodes-Param bei Photon (nur Post-Filter moeglich) → nie schlechter als
// ohne Hinweis. Pure → unit-testbar.
function preferCountry(results, cc) {
  if (!cc || !Array.isArray(results)) return results || [];
  const m = results.filter(r => r.countrycode === cc);
  return m.length ? m : results;
}
    ...
    try {
      if (provider === 'photon') {
        return await _schedule(async () => {
          ...
          // Photon hat keinen countrycodes-Param → aufgeloesten Laendercode per
          // Post-Filter anwenden (Nominatim biast via params.countrycodes).
          return preferCountry(parsePhotonResults(fc), cc);
        }, minInterval);
      }
      return await _schedule(async () => {
...
module.exports = { geocode, parseToponym, parseNominatimResults, parsePhotonResults, preferCountry };
