  ...
  try {
    if (provider === 'photon') {
      ...
      return await _schedule(async () => {
        const params = new URLSearchParams({ q, limit: '5', lang: safeLang });
        const fc = await _fetchJson(`${base}/api?${params.toString()}`);
        return parsePhotonResults(fc);
      }, minInterval);
    }
    return await _schedule(async () => {
      const params = new URLSearchParams({ q, format: 'jsonv2', limit: '5', 'accept-language': safeLang });
 ...
 */
async function geocodeAllBooks() {
  ...
  for (const book of books) {
    ...
    for (const row of rows) {
      ...
      if (!name) continue;
      attempted++;
      const candidates = await geocode(name, { lang, region });
...
module.exports = { geocode, geocodeAllBooks, parseNominatimResults, parsePhotonResults };
