...
async function geocodeAllBooks({ aiResolve = null } = {}) {
  ...
  for (const book of books) {
    ...
    for (const row of rows) {
      ...
      if (!name) continue;
      attempted++;
      let candidates = await geocode(name, { lang, region });
      // KI-Fallback: Heuristik leer → Label auf realen Toponym normalisieren,
      // dann erneut geocoden. Nur wenn ein Resolver injiziert wurde.
      if (!candidates.length && typeof aiResolve === 'function') {
        try {
          const r = await aiResolve(name, { language: lang, region });
          if (r?.ort) candidates = await geocode(r.ort, { lang, region: r.land || region });
        } catch (e) {
          logger.warn(`[geocode] KI-Fallback fehlgeschlagen fuer «${name}»: ${e.message}`);
        }
      }
