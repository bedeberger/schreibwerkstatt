...
// als einfaches zentriertes <img>. Dateiname immer cover.jpg bzw. .png.
function _buildCoverXhtml(coverData) {
  const ext = /png/i.test(coverData.mime) ? 'png' : 'jpg';
...
async function buildEpub({ scope, book, chapter, page, groups }, opts = {}) {
  ...
  // Vollbild-Cover-Seite (SVG-Wrapper haelt das Seitenverhaeltnis im Reader).
  // Erste Frontmatter-Seite, aus der TOC ausgeschlossen.
  const coverFrontmatter = coverData ? [{
    title: 'Cover',
    content: _buildCoverXhtml(coverData),
    filename: 'front_cover.xhtml',
    __level: 0,
    __toc: false,
    beforeToc: true,
  }] : [];

  // Frontmatter (Cover → Titelseite/Impressum/Widmung/Motto) vor, Autor-Bio nach
  // dem Inhalt. Alle aus der custom-TOC ausgeschlossen (__toc:false).
  const allChapters = [
    ...coverFrontmatter,
...
${tocBody}
  ...
  if (coverData) {
    const ext = /png/i.test(coverData.mime) ? 'png' : 'jpg';
    cover = new File([coverData.buffer], `cover.${ext}`, { type: coverData.mime });
  }
