...
const APP_GENERATOR = `Schreibwerkstatt ${require('../../package.json').version}`;
...
// Vollbild-Cover-Seite. Bei bekannten Bildmaßen via SVG-viewBox (haelt das
// Seitenverhaeltnis ohne Verzerrung, EPUBCheck-konform); ohne Maße (Roh-Fallback)
// als einfaches zentriertes <img>.
function _buildCoverXhtml(coverData) {
  ...
  if (coverData.width > 0 && coverData.height > 0) {
    ...
    return `<div class="cover-page"><svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" width="100%" height="100%" viewBox="0 0 ${w} ${h}" preserveAspectRatio="xMidYMid meet"><image width="${w}" height="${h}" xlink:href="${href}"/></svg></div>`;
  }
  return `<div class="cover-page"><img src="${href}" alt="Cover"/></div>`;
}

// Injiziert die Vollbild-Cover-Seite direkt in die fertige EPUB-ZIP — am
// Content-Sanitizer von epub-gen-memory vorbei (der lowercased Attribute und
// damit SVG viewBox zerstoert). Fuegt OEBPS/front_cover.xhtml hinzu und ergaenzt
// im content.opf das Manifest-Item + die Spine-Position (als erste Leseseite).
// Das Cover-Bild selbst (OEBPS/cover.<ext>) hat die Lib bereits eingebettet.
async function _injectCoverPage(buffer, coverData) {
  ...
  opf = opf.replace(
    /(<item id="css"[^>]*\/>)/,
    `$1\n        <item id="cover-page" href="front_cover.xhtml" media-type="application/xhtml+xml" />`,
  );
  // Spine: Cover als allererste Leseseite.
  opf = opf.replace(/<spine([^>]*)>/, `<spine$1>\n        <itemref idref="cover-page" />`);

  const oebpsDir = opfPath.replace(/content\.opf$/, '');
  zip.file(`${oebpsDir}front_cover.xhtml`, _buildCoverXhtml(coverData));
...
async function buildEpub({ scope, book, chapter, page, groups }, opts = {}) {
  ...
  const chapterLabel = (depth) => {
    ...
    return _chapterLabelNested(numbering, numCounters, dd, numberingMode, lang);
  };
  const epubChapters = [];
  ...
  groups.forEach((g, gi) => {
    ...
    const label = ch ? chapterLabel(d) : null;
    const withLabel = (name) => (label ? `${label}. ${name}` : name);
    if (ch && g.pages.length > 1) {
      const chTitle = withLabel(ch.name);
      epubChapters.push({
        title: chTitle,
        content: headWrap(`<h1>${escXml(chTitle)}</h1>`),
      ...
      g.pages.forEach((x, pi) => {
        epubChapters.push({
          title: x.p.name,
          content: _applyBreaks(x.pd.html, sceneSep, indentActive),
      ...
      const x = g.pages[0];
      const title = ch ? withLabel(ch.name) : x.p.name;
      const content = headWrap(`<h1>${escXml(title)}</h1>`) + _applyBreaks(x.pd.html, sceneSep, indentActive);
      epubChapters.push({
        title,
        content,
...
${tocBody}
  ...
  return coverData ? _injectCoverPage(buffer, coverData) : buffer;
}

module.exports = { buildEpub, _resolveEpubMeta, _countUnfetchableImages, _buildFrontmatter, _buildBackmatter, _proseToXhtml, _buildOpfExtraMeta, _buildAccessibilityMeta, _buildLandmarksNav, _buildContentOPF, _buildCoverXhtml, _buildCss, _applyBreaks };
