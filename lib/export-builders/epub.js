...
function _buildLandmarksNav(bodyStartFile, lang) {
...
async function buildEpub({ scope, book, chapter, page, groups }, opts = {}) {
  ...
  groups.forEach((g, gi) => {
    ...
    // Label vorab ziehen (mutiert die Counter in Dokumentreihenfolge), auch wenn
    // ein Kapitel-Intro die generierte Ueberschrift ersetzt — sonst springt die
    // Numerierung. Solo-Seiten (kein ch) ziehen kein Label.
    ...
    if (ch && g.pages.length > 1) {
      ...
      epubChapters.push({
        title: chTitle,
        content: headWrap(introHtml || `<h1>${escXml(chTitle)}</h1>`),
      ...
      const heading = ch ? (introHtml || `<h1>${escXml(title)}</h1>`) : `<h1>${escXml(x.p.name)}</h1>`;
      const content = headWrap(heading) + _applyBreaks(x.pd.html, sceneSep, indentActive);
      epubChapters.push({
        title,
        content,
...
module.exports = { buildEpub, _resolveEpubMeta, _countUnfetchableImages, _buildFrontmatter, _buildBackmatter, _proseToXhtml, _buildOpfExtraMeta, _buildAccessibilityMeta, _buildLandmarksNav, _buildContentOPF, _buildCss, _applyBreaks };
