...
.epub-chapter-title--numbered { text-align: center; margin: 2.4em 0 2.2em; line-height: 1.3; }
...
function _buildCoverXhtml(coverData, lang = 'de') {
  ...
  if (coverData.width > 0 && coverData.height > 0) {
    ...
    body = `<div class="cover-page"><svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" width="100%" height="100%" viewBox="0 0 ${w} ${h}" preserveAspectRatio="xMidYMid meet"><image width="${w}" height="${h}" xlink:href="${href}"/></svg></div>`;
  } else {
    body = `<div class="cover-page"><img src="${href}" alt="Cover"/></div>`;
  }
...
async function _injectCoverPage(buffer, coverData, lang) {
  ...
  zip.file(`${oebpsDir}front_cover.xhtml`, _buildCoverXhtml(coverData, lang));
...
async function buildEpub({ scope, book, chapter, page, groups }, opts = {}) {
  ...
  const chapterHeadingHtml = (label, name) => {
    const safe = escXml(name);
    if (!label) return `<h1>${safe}</h1>`;
  ...
  groups.forEach((g, gi) => {
    ...
    if (ch && g.pages.length > 1) {
      ...
      epubChapters.push({
        title: chTitle,
        content: headWrap(chapterHeadingHtml(label, ch.name)),
      ...
      g.pages.forEach((x, pi) => {
        epubChapters.push({
          title: x.p.name,
          content: _dedupeIds(`<h2>${escXml(x.p.name)}</h2>` + _applyBreaks(x.pd.html, sceneSep, indentActive)),
      ...
      const content = _dedupeIds(headWrap(chapterHeadingHtml(label, headingName)) + _applyBreaks(x.pd.html, sceneSep, indentActive));
      epubChapters.push({
        title,
        content,
  ...
  // Reihenfolge (vom Autor gepflegt).
...
${tocBody}
  ...
  return coverData ? _injectCoverPage(buffer, coverData, lang) : buffer;
}

module.exports = { buildEpub, _resolveEpubMeta, _countUnfetchableImages, _buildFrontmatter, _buildBackmatter, _buildExtraSections, _proseToXhtml, _buildOpfExtraMeta, _buildAccessibilityMeta, _buildLandmarksNav, _buildContentOPF, _buildCoverXhtml, _buildCss, _applyBreaks, _dedupeIds };
