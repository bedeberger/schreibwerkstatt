...
async function buildEpub({ scope, book, chapter, page, groups }, opts = {}) {
  ...
  groups.forEach((g, gi) => {
    ...
    if (ch && g.pages.length > 1) {
      ...
      const content = _dedupeIds(headWrap(`<h1>${escXml(title)}</h1>`) + _applyBreaks(x.pd.html, sceneSep, indentActive));
      epubChapters.push({
        title,
        content,
...
module.exports = { buildEpub, _resolveEpubMeta, _countUnfetchableImages, _buildFrontmatter, _buildBackmatter, _proseToXhtml, _buildOpfExtraMeta, _buildAccessibilityMeta, _buildLandmarksNav, _buildContentOPF, _buildCoverXhtml, _buildCss, _applyBreaks, _dedupeIds };
