...
function cleanPageHtml(html) {
  ...
  const out = stripTrailingEmptyBlocks(
    stripBlockEdgeNbsp(
      collapseEmptyBlocks(wrapOrphanBlocks(flattenDivBlocks(html)))
    )
  );
  return out || '<p></p>';
}

module.exports = { cleanPageHtml, wrapOrphanBlocks, collapseEmptyBlocks, stripTrailingEmptyBlocks, stripBlockEdgeNbsp, flattenDivBlocks };
