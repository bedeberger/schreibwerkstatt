...
function collectMetrics() {
  ...
  const tokenRows = db.prepare(`
    SELECT provider, model,
           ...
           COALESCE(SUM(cache_creation_1h_in),0) AS c_w_1h
      FROM (
        -- Chat-Typen aus job_runs ausgeschlossen: ihr Verbrauch kommt aus
        -- chat_messages (zweiter UNION-Zweig), sonst Doppelzaehlung.
        SELECT provider, model, tokens_in, tokens_out, cache_read_in, cache_creation_in, cache_creation_1h_in
          FROM job_runs
         WHERE ${excludeChatSourcedSql()}
        UNION ALL
        SELECT cm.provider, cm.model, cm.tokens_in, cm.tokens_out,
               cm.cache_read_in, cm.cache_creation_in, cm.cache_creation_1h_in
