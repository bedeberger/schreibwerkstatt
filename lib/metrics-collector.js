...
// kumuliert seit DB-Init. Tokens + Cost kommen aus dem persistenten Kosten-
// Ledger (ai_cost_ledger) mit eingefrorener usd — damit sind die Counter echt
// monoton kumulativ und sinken nicht mehr, wenn der 30-Tage-Prune job_runs
// leert.
...
function collectMetrics() {
  ...
  emit('sw_stt_seconds_today', 'gauge',
       'Diktat-Sekunden heute (stt_time, app.timezone)',
       db.prepare('SELECT COALESCE(SUM(seconds),0) n FROM stt_time WHERE date = ?').get(today).n);
  emit('sw_stt_chars_today', 'gauge',
       'Diktierte Zeichen heute (stt_time, app.timezone)',
       db.prepare('SELECT COALESCE(SUM(chars),0) n FROM stt_time WHERE date = ?').get(today).n);
  ...
  // ── Tokens + Cost (aus ai_cost_ledger, aggregiert pro provider/model) ───────
  // usd ist pro Ledger-Zeile zur Call-Zeit eingefroren → SUM(usd) statt Re-Compute.
  // Job-/Chat-Trennung ist bereits an der Schreib-Seite erledigt (kein Doppelzaehlen).
  ...
  for (const r of tokenRows) {
    ...
    emit('sw_cost_usd_total', 'counter',
         'Kumulierte API-Kosten in USD pro Provider/Modell (eingefroren im Kosten-Ledger)',
         r.usd, labels);
  }
