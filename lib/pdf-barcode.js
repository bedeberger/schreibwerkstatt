...
// Gesamtmass des gezeichneten Symbols in pt (Quiet Zones + menschenlesbare
// Zeile), rein aus den opts berechnet — ohne zu zeichnen. Aufrufer nutzen es,
// um Platz zu reservieren oder den Code in einer Fläche zu zentrieren. SSoT für
// die Geometrie: drawEan13 gibt exakt dieses Ergebnis zurück.
function measureEan13(opts = {}) {
  const scale = opts.scale > 0 ? opts.scale : 1.0;
  const x1 = X_MODULE_MM * MM_TO_PT * scale;
  const barH = (opts.height > 0 ? opts.height : 22.85) * MM_TO_PT * scale;
  const guardExtend = 5 * x1;
  const fontSize = Math.max(6, 9 * scale);
  const textGap = 1 * x1;
  const QUIET_LEFT = 11, QUIET_RIGHT = 7;
  return {
    width: (QUIET_LEFT + 95 + QUIET_RIGHT) * x1,
    height: barH + guardExtend + textGap + fontSize,
  };
}
...
function drawEan13(doc, x, y, ean13Raw, opts = {}) {
  ...
  return measureEan13(opts);
}

module.exports = { toEan13, checkDigit, isValidEan13, encodeModules, drawEan13, measureEan13, normalizeDigits };
