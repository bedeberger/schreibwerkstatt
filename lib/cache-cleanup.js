...
function runCacheCleanup({ vacuum = false } = {}) {
  ...
  for (const policy of POLICIES) {
    ...
    try {
      const removed = _deleteOlderThan(policy);
      summary.totalRemoved += removed;
      summary.tables.push({ table: policy.table, removed, ttlDays: policy.ttlDays });
      if (removed > 0) {
        logger.info(`[cache-cleanup] table=${policy.table} removed=${removed} ttlDays=${policy.ttlDays}`);
      }
    } catch (err) {
