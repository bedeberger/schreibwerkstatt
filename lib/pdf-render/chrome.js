...
function _drawHeaderFooter(doc, layout, ctx, outerMargins) {
  ...
  const pick = (key) => {
    if (isVerso && layout[key + 'Verso'] != null && String(layout[key + 'Verso']).length > 0) {
      return layout[key + 'Verso'];
    }
    return layout[key];
  };
  ...
  if (!skipHeader) writeRow(headerL, headerC, headerR, headerY);
  if (!skipFooter) writeRow(footerL, footerC, footerR, footerY);
