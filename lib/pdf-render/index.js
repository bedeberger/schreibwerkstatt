...
async function renderPdfBuffer({ book, groups, profile, coverBuf, authorImageBuf, token, lang, scope = 'book', chapter, page, meta }) {
  ...
  // Impressum). Früh deklariert, weil das Recto-Padding vor Widmung/TOC bereits
  // eine Leerseite registrieren kann.
  ...
  // Widmung (optional, vor TOC + Body). Auf rechte (recto = ungerade, 1-basiert)
  // Seite zwingen: würde sie sonst auf einer geraden (verso) Seite beginnen, eine
  // leere Verso-Seite davor einschieben (Buchkonvention, analog TOC). Parität
  // absolut ab Seite 1 (inkl. Cover). Nur bei scope='book' (Titelei-Zone).
  if (frontMatterAllowed && config.extras.dedication && config.extras.dedicationOnRecto
      && doc.bufferedPageRange().count % 2 === 1) {
    const blankIdx = doc.bufferedPageRange().start + doc.bufferedPageRange().count;
    doc.addPage();
    blankPageIdxs.add(blankIdx);
  }
  ...
  // TOC auf rechte (recto = ungerade, 1-basiert) Seite zwingen: würde die TOC
  // sonst auf einer geraden (verso) Seite beginnen, eine leere Verso-Seite
  // davor einschieben. Parität absolut ab Seite 1 (inkl. Cover), konsistent mit
  // dem Kapitel-Recto-Break. Nur bei scope='book' (Titelei-Zone).
  if (frontMatterAllowed && tocEffective.enabled && config.toc.startOnRecto
      && doc.bufferedPageRange().count % 2 === 1) {
    const blankIdx = doc.bufferedPageRange().start + doc.bufferedPageRange().count;
    doc.addPage();
    blankPageIdxs.add(blankIdx);
  ...
  // (bei Bleed) hier explizit setzen, analog zum onPageAdded-Hook.
  if (config.print?.padToEvenPages && doc.bufferedPageRange().count % 2 === 1) {
    doc.addPage();
