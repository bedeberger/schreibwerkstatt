 ...
 */
async function renderPdfBuffer({ book, groups, profile, coverBuf, authorImageBuf, token, lang, scope = 'book', chapter, page }) {
  ...
  if (hasInset || mirror) doc.on('pageAdded', onPageAdded);
  ...
  if (hasInset || mirror) doc.removeListener('pageAdded', onPageAdded);
  ...
  {
    let cnt = layout.pageNumberStart - 1;
    let chIdx = 0;
    let ptIdx = 0;
  ...
  const totalBodyPages = Array.from(pageNumByIdx.values()).filter(v => v != null).length;
  ...
  for (let i = bodyStartPageIdx; i < range.start + range.count; i++) {
    ...
    const pageNum = pageNumByIdx.get(i); // null fuer geskippte Pages → {page} bleibt leer
