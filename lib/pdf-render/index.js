...
const { MM_TO_PT, _pageSize, _romanize, _chapterLabel, _chapterLabelNested } = require('./layout');
...
async function renderPdfBuffer({ book, groups, profile, coverBuf, authorImageBuf, token, lang, scope = 'book', chapter, page, meta }) {
  ...
  if (config.print?.blackTextKOnly && !config.pdfa.enabled) _patchBlackToK(doc);
  ...
  // Impressum). Früh deklariert, weil das Recto-/Verso-Padding vor
  // Impressum/Widmung/TOC bereits eine Leerseite registrieren kann.
  const blankPageIdxs = new Set();
  // Recto/Verso-Paritätspflege (Druck-/Bindungskonvention, scope='book'):
  // Titelei-Seiten auf die richtige Buchseite zwingen — Impressum links (Verso),
  // Widmung + Inhaltsverzeichnis + erstes Kapitel rechts (Recto). recto =
  // ungerade 1-basierte Seite = gerader 0-basierter Index; Parität absolut ab
  // Seite 1 (inkl. Cover). Steht die nächste Seite auf der falschen Seite, wird
  // eine leere Seite (ohne Header/Footer/Nummer) eingeschoben. Jede Regel ist
  // pro Profil abschaltbar; der Aufrufer prüft das Config-Flag vor dem Call.
  const _padToSide = (wantRecto) => {
    if (!frontMatterAllowed) return;
    const nextIdx = doc.bufferedPageRange().start + doc.bufferedPageRange().count;
    if ((nextIdx % 2 === 0) === wantRecto) return;
    doc.addPage();
    if (bleedPt > 0) setPageBoxes();
  ...
  if (frontMatterAllowed && imprintPos === 'front' && _imprintHasContent(config)) {
    if (config.extras.imprintOnVerso) _padToSide(false); // Impressum links (Verso)
    _renderImprintPage(doc, config);
  }

  // Widmung (optional, vor TOC + Body) — optional auf rechte (Recto) Seite.
  if (config.extras.dedication) {
    if (config.extras.dedicationOnRecto) _padToSide(true);
  ...
  for (let bi = 0; bi < blocks.length; bi++) {
    ...
    // TOC-Level: depth 1 → 0, depth 2 → 1, depth 3 → 2. `num` bleibt vom Titel
    // getrennt, damit _renderToc die Nummern in einer eigenen Spalte
    // ausrichtet (Titel starten unabhängig von der Ziffernbreite bündig).
    tocPlan.push({ title, num, level: depth - 1, blockIdx: bi, itemIdx: -1, pageIdx: -1 });
  ...
  if (tocEffective.enabled && config.toc.startOnRecto) _padToSide(true);
  const tocPositions = tocEffective.enabled ? _renderToc(doc, tocEffective, tocPlan, docLang, config.font, margins) : [];
  ...
  if (config.chapter.firstChapterOnRecto) _padToSide(true);
  ...
  // ohne Header/Footer/Nummer. Muss NACH allen Backmatter-Seiten laufen, damit
  // die Leerseite wirklich die letzte ist, und VOR dem Nummerierungs-Pass, damit
  // blankPageIdxs sie aus der Zählung nimmt. Nach removeListener → Page-Boxes
  // (bei Bleed) hier explizit setzen, analog zum onPageAdded-Hook. Optional.
  ...
  {
    // pageCountMode='physical': alle physischen Seiten zählen mit → gedruckte
    // Zahl = PDF-Seite. Die Titelei (alles vor bodyStartPageIdx) fliesst als
    // Offset in den Body-Zähler ein; Body-Leerseiten zählen unten ebenfalls mit.
    // Titelei + Leerseiten kriegen selbst keinen Footer (Pass läuft ab
    // bodyStartPageIdx bzw. überspringt blankPageIdxs), verbrauchen aber je einen
    // Zählschritt, damit die Nummerierung lückenlos der Blattfolge folgt.
    ...
    for (let i = bodyStartPageIdx; i < range.start + range.count; i++) {
      ...
      if (blankPageIdxs.has(i)) {
        // 'physical': Leerseite verbraucht einen Zählschritt (kein Footer, aber
        // die Blattfolge bleibt lückenlos). 'body': Leerseite fällt aus der
        // Zählung (Buchkonvention).
  ...
  if (tocEffective.enabled && tocEffective.showPageNumbers && tocPositions.length === tocPlan.length) {
    ...
    for (let i = 0; i < tocPlan.length; i++) {
      ...
      // Feste (ungespiegelte) Rechtsmarge: das TOC ist eine durchgehende Spalte,
      // die bei mirrorMargins NICHT pro Verso/Recto-Seite den Bundsteg tauscht —
      // sonst springt die Seitenzahl-Kante zwischen den TOC-Seiten (analog baseX
      // in _renderToc).
  ...
  // Titelei-Nummerierungs-Pass (optional, eigener Zählstrang). Nummeriert die
  // Seiten zwischen Cover und Body-Start (Titel/Widmung/Motto/TOC) — Cover nie
  // (Full-Bleed), leere/Impressum-Seiten (blankPageIdxs) auch nicht. Format
  // 'roman' → i, ii, iii (Buchkonvention), 'arabic' → 1, 2, 3. Nur Footer,
  // kein laufender Header. Sichtbar erst ab frontMatterNumberFirstVisible;
  // frühere Seiten zählen im Strang mit, zeigen aber keine Nummer.
  const fmNumbering = layout.frontMatterNumbering; // 'none' | 'roman' | 'arabic'
  if ((fmNumbering === 'roman' || fmNumbering === 'arabic')) {
    ...
    for (let i = fmStartIdx; i < bodyStartPageIdx; i++) {
      ...
      doc.switchToPage(i);
      const isVerso = _isVersoPageIdx(i);
  ...
  // Kapitel-Endseiten: pro Kapitel die letzte nicht-leere Body-Seite (letzte
  // Seite vor dem nächsten Kapitel bzw. letzte Body-Seite beim letzten Kapitel).
  // Analog zum Start-Set, aber rückwärts gesucht, damit leere Verso-/Impressum-
  ...
  for (let i = bodyStartPageIdx; i < range.start + range.count; i++) {
    ...
    // Recto/Verso für den Header-/Footer-Slot-Pick ist eine physische
    // Seiteneigenschaft (0-basiert: gerade = recto, ungerade = verso) und
    // unabhängig von mirrorMargins.
    const isVerso = _isVersoPageIdx(i);
    _drawHeaderFooter(doc, layout, {
      title: book.name || '',
      author,
  ...
  // Leerseiten, eingeschobener Recto/Verso-Leerseiten und der padToEvenPages-
  // Auffüllseite) — die Zahl, die eine Druckerei/KDP für die Rückenbreite und
  // den Bundsteg zählt. Der Aufrufer spiegelt sie in coverSpec.pageCount.
