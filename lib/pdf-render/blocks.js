...
// verwechselt man die Strophengrenze sonst mit einem Versumbruch.
...
async function _renderBlock(doc, block, ctx) {
  const { font, indent = 0, imageCache, dropCapHint, firstParaHint, bodyFirstLineIndentPt = 0, hangingIndentPt = 0, textRole = 'body', columns = 1, columnGap = 0, hyphenate = null, widowOrphanControl = true, skipWidowOrphan = false, dpiWarnThreshold = 0, dpiWarnings = null, footnotes = null } = ctx;
  ...
  if (block.kind === 'poem' || block.kind === 'pre') {
    ...
    // Gedicht eingerückt wie das Blockzitat (ohne Strich): kursiv allein hebt
    // es im Kursivsatz eines Romans nicht vom Fliesstext ab. Einzug über
    // margins.left + pageAdded — pdfkit setzt die Spalte nach einem Auto-
    // Umbruch auf margins.left zurück (Begründung am Blockzitat oben).
    const poemInset = block.kind === 'poem' ? POEM_INDENT_PT : 0;
    const origLeft = doc.page.margins.left;
    const insetLeft = doc.x + poemInset;
    const onPoemPage = () => { doc.page.margins.left = insetLeft; };
    if (poemInset) {
      doc.page.margins.left = insetLeft;
      doc.x = insetLeft;
      doc.on('pageAdded', onPoemPage);
    }
    const leave = () => {
      if (!poemInset) return;
      doc.off('pageAdded', onPoemPage);
      doc.page.margins.left = origLeft;
      doc.x = origLeft;
    };
    if (noteLine) {
      for (const line of block.lines) {
        if (!line.length) { doc.moveDown(STANZA_GAP_LINES); continue; }
    ...
    const blankGap = block.kind === 'poem' ? STANZA_GAP_LINES : 0.4;
  ...
  if (block.kind === 'image') {
    const fetched = await _fetchImage(block.src, imageCache);
    if (!fetched) return;
