...
function _renderToc(doc, toc, entries, lang, font, baseMargins) {
  if (!toc.enabled) return [];
  doc.addPage();
  const fallback = _TOC_DEFAULT_TITLE[lang] || _TOC_DEFAULT_TITLE.de;

  // Feste (ungespiegelte) Content-Ränder: das TOC ist eine durchgehende Spalte.
  // Bei mirrorMargins darf sie NICHT pro Verso/Recto-Seite den Bundsteg tauschen
  // — sonst rückt eine mehrseitige TOC von Seite zu Seite ein/aus. Fallback auf
  // die aktuellen Page-Margins, falls kein baseMargins übergeben wird.
  const marginLeft  = baseMargins?.left  ?? doc.page.margins.left;
  const marginRight = baseMargins?.right ?? doc.page.margins.right;
  ...
  for (let ci = 0; ci < entries.length; ci++) {
    ...
    const yBefore = doc.y;
    doc.text(c.title, titleX, yBefore, {
      width: usableW,
      lineGap,
      ellipsis: true,
      lineBreak: false,
    ...
    const finalPageIdx = tocPageIdxAfter;
    const finalY = (tocPageIdxAfter !== tocPageIdxBefore) ? doc.page.margins.top : yBefore;

    // Nummer rechtsbündig in der Nummern-Spalte auf derselben Zeile wie der
    // Titel. doc.text advanced doc.y — deshalb danach auf den Stand nach dem
    // Titel-Write zurücksetzen, sonst entsteht eine Leerzeile.
    ...
    if (leader !== 'none' && toc.showPageNumbers && reservePt > 0) {
      ...
      const titleWidth = Math.min(doc.widthOfString(c.title), usableW);
