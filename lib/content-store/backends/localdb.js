...
// effektives updated_at = COALESCE(local_updated_at, updated_at), identisch zu
// dem, was loadPage als `updated_at` exponiert.
function pagesChangedSince(bookId, cursor, limit = 200) {
  ...
  const rows = db.prepare(`
    SELECT page_id AS id, COALESCE(local_updated_at, updated_at) AS updated_at
     ...
     ORDER BY updated_at ASC, page_id ASC
     LIMIT ?
  `).all(bookId, since, since, since, sinceId, lim);
  return rows;
}
