'use strict';
// Phase 1 (BookStack-Exit, docs/bookstack-exit.md): localdb-Variante der
// Content-Store-Facade. Liest/Schreibt ausschliesslich lokale SQLite-Tabellen
...
// vergibt fuer Neu-Items IDs >= 1_000_001 dank Wasserzeichen aus Phase 0.
//
...
function _pageMetaRow(r) {
  ...
  return {
    ...
    book_slug: r._book_slug || null,
    position: r.position ?? r.priority ?? null,
    updated_at: r.local_updated_at || r.updated_at || r.remote_updated_at || null,
    created_at: r.updated_at || null,
    draft: false,
    template: false,
  };
...
const _chaptersByBookStmt = db.prepare(`
  SELECT c.chapter_id, c.book_id, c.chapter_name, c.slug, c.description,
         c.position, c.priority, c.updated_at, b.slug AS _book_slug
    FROM chapters c
...
async function createChapter({ book_id, name, position, description }, _ctx) {
  const now = _nowIso();
  const result = db.prepare(`
    INSERT INTO chapters (book_id, chapter_name, description, position, priority, updated_at)
    VALUES (?, ?, ?, ?, ?, ?)
  `).run(
    book_id,
    name,
...
const _pagesByBookStmt = db.prepare(`
  SELECT p.page_id, p.book_id, p.chapter_id, p.page_name, p.slug,
         p.position, p.priority, p.updated_at, p.local_updated_at, p.remote_updated_at,
...
async function savePage(pageId, body, _ctx) {
  // TODO Phase 2: page_revisions-Row vor UPDATE schreiben (source aus body.source).
  const sets = [];
  const args = [];
  if (typeof body?.html === 'string')        { sets.push('body_html = ?');     args.push(_cleanHtmlSafe(body.html)); }
  ...
  sets.push('dirty = 0');
  args.push(pageId);
  const result = db.prepare(`UPDATE pages SET ${sets.join(', ')} WHERE page_id = ?`).run(...args);
...
async function createPage({ book_id, chapter_id, name, html }, _ctx) {
  ...
  const result = db.prepare(`
    INSERT INTO pages (book_id, chapter_id, page_name, body_html, updated_at, local_updated_at)
    VALUES (?, ?, ?, ?, ?, ?)
  `).run(book_id, chapter_id || null, name || '', cleanHtml, now, now);
...
// ── Search ──────────────────────────────────────────────────────────────────
// Phase 7 ersetzt LIKE durch FTS5; bis dahin simple Substring-Suche auf
// page_name + body_html. ACL via book_id-Param (Caller filtert separat).
