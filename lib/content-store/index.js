...
// Beschreibt den schreibenden Client fuer die Revision: nativer macOS-Client
// (Device-Token-Auth → Geraetename + Plattform aus dem Token) bzw. Browser
// (User-Agent → "<Browser> · <OS>"). null fuer server-seitige Schreiber ohne
// Request-Kontext (Cron/Jobs).
function _clientFromCtx(ctx) {
  ...
  if (u?.via === 'device_token') {
    try {
      ...
      const platform = dev?.platform || 'macOS';
...
module.exports = {
  ...
  listPages, loadPage, savePage, createPage, deletePage, backfillBlockIds,
