...
const DEFAULTS = {
  ...
  // Anti-Loop für llama.cpp, siehe ai.ollama.repeat_penalty.
  'ai.llama.repeat_penalty':   1.15,
...
const VALIDATORS = {
  ...
  // KI
  'ai.provider':                        { type: 'enum',   oneOf: ['claude', 'ollama', 'llama'] },
  ...
  'ai.llama.temperature':               { type: 'number', min: 0,    max: 2     },
  'ai.llama.context_window':            { type: 'int',    min: 2048, max: 2000000 },
  'ai.llama.max_tokens_out':            { type: 'int',    min: 512,  max: 200000 },
  'ai.llama.repeat_penalty':            { type: 'number', min: 1,    max: 2     },
