...
// Per-Request-Selbstidentifikation des Clients. Anders als `device_tokens.platform`
// (statisch am Token) beschreiben diese Header das TATSAECHLICH anfragende Geraet
// und bleiben korrekt, wenn dasselbe Token auf mehreren Geraeten (Mac + Android)
// genutzt wird. Fehlen sie, faellt das Revision-Label auf die statischen
// Token-Felder zurueck (siehe lib/content-store#_clientFromCtx).
function _headerStr(req, name, max) {
...
// Gibt `{ email, name, role, via:'device_token', tokenId, scopes,
// clientPlatform, clientDevice }` oder null.
function tryDeviceAuth(req) {
  ...
  return {
    ...
    tokenId: row.id,
    scopes: row.scopes,
    clientPlatform: _headerStr(req, 'x-client-platform', 40),
    clientDevice: _headerStr(req, 'x-client-device', 100),
  };
}
