...
const LOCAL_DEV_MODE = String(process.env.LOCAL_DEV_MODE || '').toLowerCase() === 'true';
...
function getStatus() {
  ...
  if (LOCAL_DEV_MODE) {
    return { mode: 'disabled-devmode', fromEmail: cfg.fromEmail, ready: false, missing: [] };
  }
...
async function send({ to, template, ctx = {}, locale = 'de' }) {
  ...
  if (LOCAL_DEV_MODE) {
    logger.info(`mailer: LOCAL_DEV_MODE — skip template=${template} to=${to}`);
    return { sent: false, reason: 'devmode-disabled' };
  }
  ...
  const cfg = _readConfig();
  const rendered = renderTemplate(template, { ...ctx, appName: appSettings.get('app.name') || 'Schreibwerkstatt' }, locale);
