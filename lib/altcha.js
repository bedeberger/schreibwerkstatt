...
// um das Replay-Fenster eng zu halten (Rate-Limit deckt den Rest). Live aus
// app_settings (auth.altcha.challenge_ttl_min).
function _challengeTtlMs() {
  return (appSettings.get('auth.altcha.challenge_ttl_min') || 10) * 60 * 1000;
}
...
async function createPowChallenge() {
  ...
  return createChallenge({
    hmacKey: _secret(),
    maxNumber,
    expires: new Date(Date.now() + _challengeTtlMs()),
  });
}
