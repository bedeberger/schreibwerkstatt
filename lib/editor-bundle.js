...
// Die Closure-Aufloesung (specifiersOf/resolveSpecifier/buildClosure) spiegelt
// exakt die Build-Zeit-Referenz in
// schreibwerkstatt-focuseditor/scripts/bundle-editor.mjs: dieselbe Regex
// (statische + dynamische Imports), dieselbe Pfad-Logik (relative + /-absolute
// Specifier ab public/). Bare/externe Specifier werden nicht gebuendelt
// (Warnung) — im Editor-Kern nicht erwartet.
...
// Entry-Module der Import-Closure. focus.js zieht den focus/-Kern + benoetigte
// shared/-Helfer; standalone.js ist der Mount-Einstieg der Mac-Schale (wird
// NICHT von focus.js erreicht); editor-host + block-merge explizit, weil der
// Bridge-Host bzw. die 409-Aufloesung sie brauchen (CLAUDE.md).
const ENTRY_MODULES = [
  'js/editor/focus.js',
...
// Deploy = systemd-Restart). Fehlt .git → 'unknown' (wie die Build-Referenz).
let _sourceCommit;
...
async function _build() {
  ...
  const manifest = { sourceCommit: commit, jsFiles, cssFiles };
