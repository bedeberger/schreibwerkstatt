...
// Phase 11: Per-User-Provider-Resolution. Reihenfolge:
...
if (MAX_TOKENS_OUT + CONTEXT_SAFETY_MARGIN >= MODEL_CONTEXT) {
  throw new Error(
    `Fehlkonfiguration: ai.claude.max_tokens_out (${MAX_TOKENS_OUT}) + Sicherheitspuffer (${CONTEXT_SAFETY_MARGIN}) ` +
    `>= ai.claude.context_window (${MODEL_CONTEXT}). max_tokens_out ist der Output-Cap und muss deutlich kleiner ` +
...
async function callAI(userPrompt, systemPrompt, onProgress, maxTokensOverride, signal, provider, jsonSchema, assistantPrefix) {
  const messages = [{ role: 'user', content: userPrompt }];
  return callAIChat(messages, systemPrompt, onProgress, maxTokensOverride, signal, provider, jsonSchema, undefined, assistantPrefix);
}
...
//
// assistantPrefix (claude-only): String, der als finale Assistant-Message ans
// messages-Array gehängt wird (Anthropic-„Prefill"). Claude setzt die Antwort
// nahtlos nach diesem Prefix fort. Für JSON-Only-Pfade typisch `'{'` — verhindert
// Markdown-Fences und Meta-Text, macht den jsonrepair-Fallback in 99% der Fälle
// unnötig. Der Prefix wird beim Rückgabewert wieder vorne an `text` angeklebt,
// sodass parseJSON ein vollständiges JSON-Objekt sieht.
// Lokale Provider (Ollama/Llama) erzwingen JSON ohnehin per Grammar — Prefix wird
// dort ignoriert.
async function callAIChat(messages, systemPrompt, onProgress, maxTokensOverride, signal, provider, jsonSchema, temperatureOverride, assistantPrefix) {
  ...
  return _callClaude(messages, systemPrompt, onProgress, maxTokensOverride, signal, assistantPrefix);
}
...
function _claudeTimeoutMs() {
  return parseInt(appSettings.get('ai.claude.timeout_ms'), 10) || 10 * 60 * 1000;
}
...
async function _callClaude(messages, systemPrompt, onProgress, maxTokensOverride, signal, assistantPrefix) {
  const maxAttempts = _retryMaxAttempts();
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    try {
      return await _callClaudeAttempt(messages, systemPrompt, onProgress, maxTokensOverride, signal, assistantPrefix);
    } catch (e) {
...
async function _callClaudeAttempt(messages, systemPrompt, onProgress, maxTokensOverride, signal, assistantPrefix) {
    const model = appSettings.get('ai.claude.model') || 'claude-sonnet-4-6';
    const globalMax = MAX_TOKENS_OUT;
    const maxTokens = maxTokensOverride ? Math.min(maxTokensOverride, globalMax) : globalMax;
    // Anthropic-Prefill: finale assistant-Message zwingt Claude, ab dem Prefix
    // weiterzuschreiben. Trailing-Whitespace ist verboten — Caller liefert in
    // JSON-Pfaden typisch `'{'` (kein Whitespace, sicher). Wir verlängern das
    // gestreamte `text` am Ende wieder um den Prefix, damit parseJSON ein
    // komplettes Objekt sieht.
    const prefill = (typeof assistantPrefix === 'string' && assistantPrefix.length > 0)
      ? assistantPrefix.replace(/\s+$/, '')
      : '';
    ...
    const genDurationMs = (t_first && t_last > t_first) ? t_last - t_first : null;
    // Prefill wieder vorne anhängen — Stream enthält nur die Fortsetzung NACH
    // dem Assistant-Prefix; parseJSON braucht das `{` (oder was auch immer
    // prefill war), damit das Objekt syntaktisch geschlossen ist.
    const finalText = prefill ? prefill + text : text;
    return { text: finalText, truncated, tokensIn, tokensOut, cacheReadIn, cacheCreationIn, genDurationMs, stopReason, provider: 'claude', model };
    } finally {
      cleanup();
    }
}
...
// Phase 11: per-Provider Context-Config. Boot-Konstanten (`INPUT_BUDGET_TOKENS` etc.)
