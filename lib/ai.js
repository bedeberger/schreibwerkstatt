...
function llamaRepeatPenalty() {
  const v = Number(appSettings.get('ai.llama.repeat_penalty'));
  return Number.isFinite(v) && v >= 1 ? v : 1.15;
}
...
async function _callOllama(messages, systemPrompt, onProgress, maxTokensOverride, signal, jsonSchema, temperatureOverride) {
    ...
    try {
      resp = await fetch(`${host}/api/chat`, {
        ...
        body: JSON.stringify({ model, messages: allMessages, stream: true, format: fmt, options: { num_ctx, num_predict: maxTokens, temperature, repeat_penalty: ollamaRepeatPenalty(), think: false } }),
        signal,
      });
    } catch (fetchErr) {
  ...
  try {
    resp = await fetch(`${host}/v1/chat/completions`, {
      ...
      body: JSON.stringify({
        ...
        repeat_penalty: llamaRepeatPenalty(),
...
// ai.claude.context_window > 200000 gesetzt ist (nur Sonnet 4.x unterstützt 1M).
function _claudeBetaHeader() {
  const betas = ['prompt-caching-2024-07-31', 'extended-cache-ttl-2025-04-11'];
  if (MODEL_CONTEXT > 200000) betas.push('context-1m-2025-08-07');
  return betas.join(',');
}
