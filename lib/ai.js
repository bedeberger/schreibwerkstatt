...
// oder massiv unterausgelasteter Kontext. Env-Override `CHARS_PER_TOKEN` für
...
const RETRY_MAX_ATTEMPTS = parseInt(process.env.CLAUDE_RETRY_MAX, 10) || 3;
...
async function _callClaude(messages, systemPrompt, onProgress, maxTokensOverride, signal, assistantPrefix) {
  for (let attempt = 0; attempt < RETRY_MAX_ATTEMPTS; attempt++) {
    try {
      ...
      if (e?.code === 'AI_OVERLOADED' && !signal?.aborted && attempt < RETRY_MAX_ATTEMPTS - 1) {
        const delay = _retryDelayMs(attempt, e.retryAfterSec);
        logger.warn(`Claude overload (${e.status || 'stream'}), Versuch ${attempt + 1}/${RETRY_MAX_ATTEMPTS}, retry in ${Math.round(delay)}ms`);
