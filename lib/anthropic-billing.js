...
async function fetchCostReport({ fromDay, toDay, apiKey, fetchImpl = fetch }) {
  ...
  for (let i = 0; i < MAX_PAGES; i++) {
    ...
    try {
      ...
      }, TIMEOUT_MS, { fetchImpl, timeoutCode: 'BILLING_TIMEOUT' });
      ...
      const buf = await readCapped(res, MAX_BYTES, { tooLargeCode: 'BILLING_RESPONSE_TOO_LARGE' });
      body = JSON.parse(buf.toString('utf8'));
    } catch (e) {
      if (e.code) throw e;
      throw _fail(e.name === 'AbortError' || e.name === 'TimeoutError' ? 'BILLING_TIMEOUT' : 'BILLING_FETCH_FAILED');
    }
