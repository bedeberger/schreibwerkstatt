...
async function _loadChapterGroups(chapterId, ctx) {
  const chapter = await contentStore.loadChapter(chapterId, ctx);
  if (!chapter) throw _empty('CHAPTER_NOT_FOUND');
  const bookPages = await contentStore.listPages(chapter.book_id, ctx);
  ...
  return {
    chapter,
    groups: [{ chapterId: chapter.id, chapter, pages: valid }],
  };
}
...
async function loadContents({ scope, id }, ctx) {
  ...
  if (scope === 'chapter') {
    const { chapter, groups } = await _loadChapterGroups(id, ctx);
