...
// ── Streaming-Export (legacy BookStack-format-proxy) ────────────────────────

const { BOOKSTACK_URL, authHeader } = require('./bookstack');

/**
 * Liefert die `/books/:id/export/:fmt`-Antwort als Stream-Response zurueck —
 * fuer Pass-Through-Downloads (pdf/html/txt/md) ueber den BookStack-Renderer.
 * Caller (routes/export.js) pipelined die Response weiter. Liefert `{ ok,
 * status, headers, body }`-Shape, ohne den Body zu konsumieren.
 *
 * Phase 7 (Kill BookStack) ersetzt diese Methode durch eigene Renderer.
 */
async function streamExport(bookId, format, ctx) {
  return fetch(`${BOOKSTACK_URL}/api/books/${bookId}/export/${format}`, {
    headers: { Authorization: authHeader(_resolveToken(ctx)) },
  });
}
...
module.exports = {
  ...
  // Higher-level
  bookTree, loadPagesBatch, searchPages, streamExport,
