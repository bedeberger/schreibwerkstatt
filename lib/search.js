...
// HTML zu Text. Identisch zu routes/sync.js und db/page-revisions.js.
function htmlToText(html) {
  return String(html || '').replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ').trim();
}

const VALID_KINDS = new Set([
  'book', 'chapter', 'page', 'figure', 'location', 'scene', 'idea',
]);
...
function reindexAll() {
  ...
  const counts = { book: 0, chapter: 0, page: 0, figure: 0, location: 0, scene: 0, idea: 0 };
