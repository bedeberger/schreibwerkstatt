...
function _upsertResearch(itemId) {
  const r = db.prepare(
    'SELECT id, book_id, kind, title, body, url, source, doc_name, doc_text FROM research_items WHERE id = ?'
  ).get(itemId);
  ...
  const body = [r.body, r.url, r.source, r.doc_name, r.doc_text].filter(Boolean).join('\n');
