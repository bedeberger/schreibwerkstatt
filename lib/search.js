...
function _upsertResearch(itemId) {
  const r = db.prepare(
    'SELECT id, book_id, kind, title, body, url, source FROM research_items WHERE id = ?'
  ).get(itemId);
  if (!r) { remove('research', itemId); return; }
  // Plain-Text-Felder. Titel-Fallback: erste Body-Zeile (fuer Trigram + bm25-Boost).
  const body = [r.body, r.url, r.source].filter(Boolean).join('\n');
  const firstLine = (r.body || '').toString().split('\n')[0] || '';
  const title = (r.title && r.title.trim()) ? r.title.trim() : firstLine.slice(0, 120);
